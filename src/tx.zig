...
fn intrinsicGas(data: []const u8, is_create: bool, eip2: bool, eip2028: bool, eip3860: bool) Intrinsic {
    var zero: u64 = 0;
    var nonzero: u64 = 0;
    for (data) |b| {
        if (b == 0) zero += 1 else nonzero += 1;
    }
    // EIP-2 (Homestead) introduced the 32000 contract-creation intrinsic cost;
    // in Frontier a creation transaction cost only TX_BASE (21000).
    const tx_create: u64 = if (eip2) TX_CREATE else 0;
    // EIP-2028 (Istanbul) reduced the non-zero calldata byte cost from 68 to 16.
    const nonzero_cost: u64 = if (eip2028) 16 else 68;
    ...
    const standard: u64 = TX_BASE + zero * 4 + nonzero * nonzero_cost + create_extra;
...
pub fn validate(state: *State, env: *const vm.Environment, tx: Tx, max_fee_cap: u256, max_prio: u256) ?InvalidReason {
    ...
    const ig = intrinsicGas(tx.data, tx.to == null, env.fork.atLeast(.homestead), env.fork.atLeast(.istanbul), eip3860);
...
fn processImpl(allocator: std.mem.Allocator, state: *State, env: *const vm.Environment, tx: Tx, logs_out: ?*std.ArrayList(vm.Log)) Result {
    ...
    const ig = intrinsicGas(tx.data, tx.to == null, env.fork.atLeast(.homestead), env.fork.atLeast(.istanbul), env.fork.atLeast(.shanghai));
    var intrinsic = ig.standard;
    for (tx.access_list) |e| intrinsic += ACCESS_LIST_ADDRESS + ACCESS_LIST_KEY * e.keys.len;
