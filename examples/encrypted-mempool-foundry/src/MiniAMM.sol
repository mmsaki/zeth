...
/// Constant-product AMM (x*y=k, no fee) with internal per-account balances, so
/// each swap is attributed to its msg.sender and an attacker's MEV shows up as a
/// real token0 balance gain.
contract MiniAMM {
    uint256 public r0;
    uint256 public r1;
